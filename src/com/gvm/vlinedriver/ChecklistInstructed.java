	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		checknum=getIntent().getStringExtra("checknum");
			...
			if(c.moveToFirst())
			{
				...
				loadspinerfilter("1",c.getString(c.getColumnIndex("corridordown")),spcorridordown,spcorridorup.getSelectedItem().toString());
				
				loadspiner("1",c.getString(c.getColumnIndex("insfrom")),spfrom);
				loadspinerfilter("1",c.getString(c.getColumnIndex("insto")),spto,spfrom.getSelectedItem().toString());
				//loadspinerfilter("1",c.getString(c.getColumnIndex("insto")),spto,spfrom.getSelectedItem().toString());
	   ...
	   //get trainee username
		...
		if(crole.getCount()>0)
			...
			if(crole.getString(crole.getColumnIndex("role")).equalsIgnoreCase("assessor"))
			{
				...
				if(!(sptrainer.getSelectedItem().toString().equalsIgnoreCase("") || etdate.getText().toString().equalsIgnoreCase("") || spcorridordown.getSelectedItem().toString().equalsIgnoreCase("") || spcorridorup.getSelectedItem().toString().equalsIgnoreCase("")))
				{
		...
		//When we change corridor up, we need to upload corridor down
		spcorridorup.setOnItemSelectedListener(new OnItemSelectedListener() {

			public void onItemSelected(AdapterView<?> arg0, View arg1,
					int arg2, long arg3)
			{
				loadspinerfilter("1",spcorridordown.getSelectedItem().toString(),spcorridordown,spcorridorup.getSelectedItem().toString());
			}
		...
		//When we change from, we need to upload To
				spfrom.setOnItemSelectedListener(new OnItemSelectedListener() {

					public void onItemSelected(AdapterView<?> arg0, View arg1,
							int arg2, long arg3)
					{
						loadspinerfilter("1",spto.getSelectedItem().toString(),spto,spfrom.getSelectedItem().toString());
					}
			...
			public void onClick(View v) {
				...
				if(cuserrole.getCount()>0)
				{
					...
					if((cuserrole.getString(cuserrole.getColumnIndex("role")).equalsIgnoreCase("trainer")) && ((trainerinstruction==null) || (trainerinstruction.isEmpty()) || (cuserrole.getString(cuserrole.getColumnIndex("username")).equalsIgnoreCase(trainerinstruction))))
					{
						...
						//tvconduct.setText(trainerinstruction);
						if(!((sptrainer.getSelectedItem().toString().equalsIgnoreCase("") && etdate.getText().toString().equalsIgnoreCase("")) || spcorridordown.getSelectedItem().toString().equalsIgnoreCase("") || spcorridorup.getSelectedItem().toString().equalsIgnoreCase("")))
						{
							chbsig.setEnabled(true);
						}
